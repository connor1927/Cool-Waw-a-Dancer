  ...
  const handleRewardToggle = (rewardId: string) => {
    ...
    setTimeout(() => {
      setSelectedRewards((prev) => {
        const newSelection = prev.includes(rewardId)
          ? prev.filter((id) => id !== rewardId)
          : prev.length < maxSelections
            ? [...prev, rewardId]
            : prev

        // Show scroll indicator when a reward is selected
        if (newSelection.length > 0 && !prev.includes(rewardId)) {
          setShowScrollIndicator(true)
        }
  ...
  return (
    <>
      ...
      <div className={`scroll-indicator ${!showScrollIndicator ? "hidden" : ""}`}>
        <div className="glass-scroll-card relative">
          <div className="flex items-center justify-center space-x-2">
            <span className="scroll-text">Scroll Down</span>
            <ChevronDown className="w-6 h-6 scroll-arrow" />
          </div>
        </div>
      </div>
